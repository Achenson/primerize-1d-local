...
export async function executePrimerizeDesign({
  ...
  if (cleanSeq.startsWith(T7_PROMOTER)) {
    ...
    if (gCount === 0) {
      t7GWarning =
        'Sequence does not start with G after T7 promoter. In vitro transcription may fail.';
    } else if (gCount === 1) {
      t7GWarning =
        'Sequence starts with only one G after T7 promoter. Transcription may be suboptimal.';
    }
  }
  ...
  if (!strictPureRegex.test(cleanSeq)) {
    throw new Error(
      'Invalid sequence format. Internal spaces, numbers, or special characters are not allowed. Use only A, C, G, T, or U.',
    );
  }
  ...
  if (seqLength < 60) {
    throw new Error(
      `Sequence too short (${seqLength} bp). Minimum length required is 60 bp.`,
    );
  }
  if (seqLength > 1000) {
    throw new Error(
      `Sequence too long (${seqLength} bp). Maximum allowed size is 1000 bp.`,
    );
  }
  ...
  const operationalMinLength =
    minLength === '' ? 15 : Math.min(60, Math.max(10, Number(minLength)));
  const operationalMinTm =
    minTm === '' ? 60 : Math.min(80, Math.max(50, Number(minTm)));
  const operationalNumPrimers =
    numPrimers === '' ? null : Math.min(50, Math.max(2, Number(numPrimers)));
  ...
  if (scriptOutput.includes('Number of Primers Designed: 0')) {
    throw new Error(
      'No valid assembly found. The engine cannot satisfy the current thermodynamic constraints. Please adjust your design parameters.',
    );
  }
